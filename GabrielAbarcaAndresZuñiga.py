...
def load_music(filename):#Función para la música
        music = pygame.mixer.music.load(filename)
        return music#Retorna la música
...
def readtxt(archivo):#Lector de TXT
        ...
        archi.close()
        return save
def readrestxt(archivo):#Lector de TXT
        archi=open(archivo,'r')#lee el archivo
        linea=archi.readlines()
...
def readresstxt(archivo):#Lector de TXT
        archi=open(archivo,'r')#lee el archivo
        linea=archi.readlines()
        save1=linea[0][:-1]#guarda la primera linea menos el "enter"
        save2=linea[1][:-1]
...
def portero():#agarra número aleartorios para usarlos como el portero
    dif=readtxt("Memoria/Dificultad.txt")
    lista=[1,2,3,4,5,6]
...
def comparador(portero,compuerta):#Compara que el número de la compuerto esté dentro de los numeros elegidos
    if portero==[]:
        return True
    elif compuerta==portero[0]:
        return False
    elif compuerta!=portero[0]:
        return comparador(portero[1:],compuerta)
...
def ahoratirando(jugador,turno):#Función que debería mostrar el artillero, el portero, penales cobrados, fallados, y retornaría a mostrar si hubo Gol o no
    ...
    if turno==2:
        ...
        pen1j1,pen1j1_rect=texto(convertir(res1[0]),500,HEIGHT*2/3)
        pen1j2,pen1j2_rect=texto(convertir(res2[0]),500,HEIGHT*2/3)
    elif turno==3:
        res1=readresstxt("Memoria/Jugador1-Score.txt")
        res2=readresstxt("Memoria/Jugador2-Score.txt")
        pen1j1,pen1j1_rect=texto(convertir(res1[0]),500,HEIGHT*2/3)
        pen1j2,pen1j2_rect=texto(convertir(res2[0]),500,HEIGHT*2/3)
    ...
    while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key==K_KP_ENTER:
                        if jugador==1:
                            if turno==1:
                                grabartxt("Memoria/Jugador1-Score.txt",str(gol))
                            elif turno !=1:
                                agregartxt("Memoria/Jugador1-Score.txt",str(gol))
                            return main(9)#Retorna al main
                        elif jugador==2:
                            global tirador
                            if turno==1:
                                grabartxt("Memoria/Jugador2-Score.txt",str(gol))
                            elif turno !=1:
                                agregartxt("Memoria/Jugador2-Score.txt",str(gol))
                            if turno==3:
                                tirador=1
                                return main(10)
                            elif turno!=0:
            ...
            if jugador==1:
                if turno==2:
                    screen.blit(pen1j1,pen1j1_rect)
                elif turno==3:
                    screen.blit(pen1j1,pen1j1_rect)
                    screen.blit(pen2j1,pen2j1_rect)
            elif jugador==2:
                if turno==2:
                    screen.blit(pen1j2,pen1j2_rect)
                elif turno==3:
                    screen.blit(pen1j2,pen1j2_rect)
                    screen.blit(pen2j2,pen2j2_rect)
            screen.blit(background_image,(0,0))
            screen.blit(logo,(0,0))
            screen.blit(penal,(WIDTH-180,0))
            screen.blit(Tittle,Tittle_rect)
            pygame.display.flip()
    return 0
def estadísticas():#Tabla de goles
    ...
    pen2j2,pen2j2_rect=texto(convertir(res2[1]),WIDTH/2,HEIGHT*2/3)
    pen3j2,pen3j2_rect=texto(convertir(res2[2]),WIDTH*3/4,HEIGHT*2/3)
    while True:
            time = clock.tick(60)
            for event in pygame.event.get():
                if event.type == QUIT:
            ...
            screen.blit(background_image,(0,0))
            screen.blit(logo1,(0,HEIGHT/3-125))
            screen.blit(logo2,(0,HEIGHT*2/3-125))
            screen.blit(pen1j1,pen1j1_rect)
            screen.blit(pen2j1,pen2j1_rect)
            screen.blit(pen3j1,pen3j1_rect)
            screen.blit(pen1j2,pen1j2_rect)
            screen.blit(pen2j2,pen2j2_rect)
            screen.blit(pen3j2,pen3j2_rect)
            pygame.display.flip()
    return 0
